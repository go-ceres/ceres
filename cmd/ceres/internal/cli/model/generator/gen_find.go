...
//go:embed tpl/find.go.tpl
var findTemplate string

func (g *Generator) genFind(table parser.Table) (string, error) {
	tplText, err := pathx.LoadTpl(category, findTemplateFile, findTemplate)
	if err != nil {
		return "", err
	}
	text, err := templatex.With("find").Parse(tplText).Execute(map[string]interface{}{
